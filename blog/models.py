...
class BlogIndexPage(RoutablePageMixin, Page):
    featured_story_title = models.CharField(max_length=255, blank=True)
    featured_story_body = RichTextField(blank=True, null=True)
    featured_story_image = models.ForeignKey(
        'wagtailimages.Image', null=True, on_delete=models.SET_NULL, related_name='+')

    date = models.DateField("Post date", null=True, blank=True)
    author = models.CharField(max_length=100, default='JLH-Uganda, Admin')

    content_panels = Page.content_panels + [

        FieldPanel('featured_story_title'),
        ImageChooserPanel('featured_story_image'),
        FieldPanel('featured_story_body'),
        FieldPanel('date'),
        FieldPanel('author'),
    ...
    def get_context(self, request, *args, **kwargs):
        ...
        all_sponsored_pages = BlogPage.objects.live(
        ).public().order_by('-last_published_at')

        # @TODO: paginate change integer to 8 per page
        paginator = Paginator(all_sponsored_pages, 3)
...
class BlogCategory(models.Model):
    ...
    icon = models.ForeignKey('wagtailimages.Image', null=True,
    ...
    class Meta:
        verbose_name_plural = 'Sories categories'
...
class BlogPage(RoutablePageMixin, Page):
    ...
    def search(self, request, *args, **kwargs):
        ...
        self.sponsored_pages = self.get_sponsored_pages(
        ).objects.live().autocomplete("categories", search_query)
        if search_query:
